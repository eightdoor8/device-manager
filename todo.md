...
- [x] WEB管理画面：フィルタ・検索機能を削除
- [x] WEB管理画面：最新100件のログをそのまま表示する仕様に変更
- [x] 修正内容をテストしてデグレがないか確認
- [ ] チェックポイントに保存
...
- [ ] Vercel プロジェクトを新規作成
- [ ] admin/ を Vercel にデプロイ
- [ ] デプロイ後の動作を確認
- [ ] QA用の固定URL を取得してQAチームに共有

## QA報告：WEB管理画面の不具合修正（現在対応中）
- [ ] 端末一覧から「削除」をクリックしても削除されない問題を修正


## iOS アプリ対応（進行中）

### Phase 1: iOS 設計・準備
- [x] iOS UI/UX デザイン文書作成（design.md）
- [x] Apple HIG に準拠したスクリーン設計
- [x] iOS 固有のナビゲーションパターン検討
- [x] iOS 固有のコンポーネント実装計画

### Phase 2: iOS UI コンポーネント実装
- [x] iOS スタイルの TabBar 実装
- [x] iOS スタイルの NavigationStack 実装
- [x] iOS スタイルのモーダル・シート実装
- [x] ハプティックフィードバック統合
- [x] iOS スタイルボタンコンポーネント実装
- [x] iOS スタイルカードコンポーネント実装
- [x] iOS ステータスバッジ実装
- [x] iOS 検索バー実装
- [x] テーマカラーを iOS HIG 准拠に更新
